...
void sortc(void);
void help(void);
size_t get_input(char *target, size_t max_size);
int last = 0;
...
size_t get_input(char *target, size_t max_size)//destination and max size can receive
{
    ...
    if (max_size > line_size) {
        str[line_size - 1] = '\0';
        strncpy(target, str, line_size);
    } else {
        str[max_size - 1] = '\0';
        strncpy(target, str, max_size);
        line_size = max_size;
    }
    free(str);
    return line_size;
...
void insert(void) {
    ...
    while (ans == 'y') {
        ...
        printf("\n\t  1-F.Name: ");
        get_input(new_contact->fname, 31);

        printf("\t  2-L.Name: ");
        get_input(new_contact->lname, 31);

        printf("\t  3-Tele.P: ");
        get_input(new_contact->telep, 15);

        printf("\t  4-Cell.P: ");
        get_input(new_contact->cellp, 15);
